...
let
  ...
  remoteMachines = map (name: machines.${name}) (
    filter (name: name != config.signal.machine.signalName) (attrNames machines)
  );
        ...
        }:
        {
          options = with lib; {
            ...
            type = mkOption {
              ...
              readOnly = true;
              default = if std.hasInfix ":" config.address then "v6" else "v4";
            };
            __toString = mkOption {
              type = types.anything;
              readOnly = true;
                  ...
                  wireguard = {
                    ...
                    addresses = mkOption {
                      ...
                      default = map (
                        ...
                        {
                          inherit address;
                          mask = if type == "v4" then 24 else 48;
                        }
                      ) config.wireguard.allowedIps;
                    };
                    endpoint = mkOption {
                      type = types.nullOr types.str;
                      default = if config.domain != "local" then "${config.fqdn}:51860" else null;
                    };
                  };
                  nix = {
                    serve = {
  ...
  config = lib.mkMerge [
    {
      signal.machines =
        let
          ...
          nixPubKeys = config.users.users.ash.openssh.authorizedKeys.keys ++ [
            # hermes
      ...
      networking.wireguard.networks."wg-signal" = {
        ...
        peers = foldl' (
          ...
          let
            mcn = machines.${name};
          in
          if name == config.signal.machine.signalName then
            peers
          else
            (
              peers
      ...
      nix = foldl' (
        ...
        let
          ...
          res =
            if build.enable then
              {
                distributedBuilds = true; # FIX :: this is usually too slow to be worth it
                buildMachines = [
                  {
                    hostName = build.fqdn;
                    protocol = "ssh-ng";
                ...
                settings =
                  if serve.enable then
                    let
                      opts = serve.extraOptions // {
                        inherit (serve) priority;
                      };
                    in
                    {
                      substituters = (acc.settings.substituters or [ ]) ++ [
    ...
    (lib.mkIf (local.nix.build.enable or false) (
      let
    ...
    (lib.mkIf (local.nix.serve.enable or false) (
      let
